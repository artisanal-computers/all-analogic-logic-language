...
- Sendo os componentes algo que tratam sinais elétricos, faz sentido usar multiplicações e divisões pois são alteradoras de proporções, mas somar e subtrair por números constantes (e não outros tipos de variaveis) pode não fazer sentido, por isso nesses casos recomenda-se ter uma entrada no escopo global `:ONE;` usada como referencia para certos cálculos. Ou seja, é importante pensar em proporções, e não em valores.
...
- Assim como a memória pode ser usada repetindo o nome de si como argumento, retro-alimentação pode ser representada usando como argumento um função que será declarada/chamada posteriormente.
- Para tratar valores que tendem a infinidade (para cima ou para baixo), deve-se pensar no limite das voltagens do circuito. Para valores que são resultado de divisão por 0, esses limites talvez sejam o retorno. Uma boa prática seria usar constantes com valores em volts, ou em multiplos, ex: `GND:0; VCC:5000;`.
